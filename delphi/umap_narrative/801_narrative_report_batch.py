...
class NarrativeReportService:
    ...
    def store_report(
        self, report_id, section, model, report_data, job_id=None, metadata=None
    ):
...
class BatchReportGenerator:
    ...
    def __init__(
        ...
        if not model:
            ...
            if not model:
                raise ValueError(
                    "Model must be specified via --model argument or ANTHROPIC_MODEL environment variable"
                )
        self.model = model
    ...
    def _get_math_main_data(self, conversation_id):
        ...
        try:
            ...
            results = self.postgres_client.query(
                sql, {"zid": conversation_id, "math_env": math_env}
            )

            if not results:
                logger.warning(
                    f"No math_main data found for conversation {conversation_id} with math_env {math_env}"
                )
                return None
            ...
            logger.info(
                f"Successfully retrieved math_main data for conversation {conversation_id}"
            )
            logger.debug(
                f"Math data keys: {list(math_data.keys()) if isinstance(math_data, dict) else 'not a dict'}"
            )
            ...
            logger.error(
                f"Error retrieving math_main data for conversation {conversation_id}: {str(e)}"
            )
            import traceback
    ...
    async def get_conversation_data(self):
        ...
        try:
            ...
            conversation = self.postgres_client.get_conversation_by_id(
                int(self.conversation_id)
            )
            if not conversation:
                logger.error(
                    f"Conversation {self.conversation_id} not found in database."
                )
                return None

            # Get comments
            comments = self.postgres_client.get_comments_by_conversation(
                int(self.conversation_id)
            )
            logger.info(
                f"Retrieved {len(comments)} comments from conversation {self.conversation_id}"
            )
            ...
            if not math_data:
                logger.warning(
                    f"No math data found in math_main for conversation {self.conversation_id}"
                )
                return None
            ...
            export_data = self.group_processor.get_export_data(
                int(self.conversation_id), self.include_moderation
            )
            ...
            logger.info(
                f"Enriched {len(processed_comments)} comments with Clojure-calculated metrics"
            )
            ...
            if enriched_count > 0:
                logger.info(
                    f"Enriched {enriched_count} comments with {total_assignments} total cluster assignments across all layers"
                )
            else:
    ...
    def load_comment_clusters_from_dynamodb(self, conversation_id):
        ...
        """
        try:
            clusters_table = self.dynamodb.Table(
                "Delphi_CommentHierarchicalClusterAssignments"
            )
            cluster_map = {}

            logger.info(
                f"Querying for cluster assignments for conversation_id: {conversation_id}"
            )
                    ...
                    if comment_id is not None:
                        ...
                        for key, value in item.items():
                            if (
                                ...
                                layer_num_str = key.replace("layer", "").replace(
                                    "_cluster_id", ""
                                )
                                try:
            ...
            logger.info(
                f"Loaded {len(cluster_map)} comment cluster assignments across {len(available_layers)} layers: {sorted(available_layers)}"
            )
            return cluster_map
        except Exception as e:
    ...
    async def get_topics(self):
        ...
        try:
            ...
            logger.info(
                f"Fetching all topic names for conversation {self.conversation_id}..."
            )
            topic_names_table = self.dynamodb.Table(
            ...
            logger.info(
                f"Fetching all structure/keyword data for conversation {self.conversation_id}..."
            )
            keywords_table = self.dynamodb.Table(
            ...
            logger.info(
                f"Created lookup map for {len(keywords_lookup)} keyword entries."
            )

            # Load all cluster assignments for all comments
            all_clusters = await asyncio.to_thread(
                self.load_comment_clusters_from_dynamodb, self.conversation_id
            )

            # --- Step 2: Process the fetched data ---

            available_layers = set(
                layer for clusters in all_clusters.values() for layer in clusters.keys()
            )
            layers_to_process = sorted(list(available_layers))
            if self.layers is not None:
                layers_to_process = [
                    layer for layer in layers_to_process if layer in self.layers
                ]
            ...
            for layer_id in layers_to_process:
                ...
                layer_topic_names = [
                    item
                    for item in topic_names_items
                    if int(item.get("layer_id", -1)) == layer_id
                ]
            ...
            if not self.job_id:
                raise ValueError(
                    "job_id is required for versioned topic keys but is missing or empty"
                )
            ...
            logger.info(
                f"Created {len(all_topics)} sections total: {len(all_topics) - len(global_sections)} layer topics + {len(global_sections)} global sections"
            )

            # Sort topics for processing
            all_topics.sort(
                key=lambda x: (
                    0 if x.get("section_type") == "global" else 1,
                    x.get("layer_id", -1),
                    -len(x.get("citations", [])),
                )
            )
            ...
            logger.error(
                f"A critical error occurred in get_topics: {str(e)}", exc_info=True
            )
            return []
    ...
    def filter_topics(
        ...
        if topic_cluster_id is not None and topic_layer_id is not None:
            ...
            if comment_cluster_id is not None:
                ...
                if str(topic_cluster_id) == "0" and comment_id in [
                    1,
                    2,
                    3,
                ]:  # Log first few comments
                    logger.info(
                        f"DEBUG: Checking comment {comment_id} - layer{topic_layer_id}_cluster_id={comment_cluster_id}, topic_cluster_id={topic_cluster_id}"
                    )
                    logger.info(
                        f"DEBUG: String comparison: '{str(comment_cluster_id)}' == '{str(topic_cluster_id)}' = {str(comment_cluster_id) == str(topic_cluster_id)}"
                    )
        ...
        if sample_comments and len(sample_comments) > 0:
            ...
            for sample in sample_comments:
                ...
                if (
                    sample.lower() in comment_text.lower()
                    or comment_text.lower() in sample.lower()
                ):
                    return True
    ...
    def _get_dynamic_comment_limit(
        self, layer_id=None, total_layers=None, comment_count=None, filter_type=None
    ):
        """
        ...
        try:
            ...
            logger.debug(
                f"Dynamic comment limit: category={category}, base={base_limits[category]}, "
        ...
        try:
            ...
            def get_sort_key(comment):
                ...
                if filter_type == "comment_extremity":
                    ...
                    total_engagement = (
                        int(agrees) + int(disagrees)
                        if isinstance(agrees, (int, float))
                        and isinstance(disagrees, (int, float))
                        else 0
                    )
            ...
            logger.info(
                f"Selected {len(selected)} high-quality comments from {len(comments)} "
            ...
            try:
                sorted_comments = sorted(
                    ...
                    key=lambda c: (
                        int(c.get("votes", 0))
                        if isinstance(c.get("votes"), (int, float))
                        else 0
                    ),
                    reverse=True,
                )
    ...
    async def get_comments_as_xml(
        self, conversation_data: dict, filter_func=None, filter_args=None
    ):
        ...
        try:
            ...
            if filter_func:
                if filter_args:
                    filtered_comments = [
                        c for c in filtered_comments if filter_func(c, **filter_args)
                    ]
                else:
            ...
            if filter_func == self.filter_topics and len(filtered_comments) > 0:
                ...
                if filter_args:
                    ...
                    if layer_id is not None:
                        ...
                        total_layers = max(
                            layer_id + 1, 3
                        )  # Assume at least 3 layers if we have layer data
                ...
                if len(filtered_comments) > comment_limit:
                    logger.info(
                        f"Applying dynamic comment limit: {len(filtered_comments)} -> {comment_limit} "
                    ...
                    filtered_comments = self._select_high_quality_comments(
                        filtered_comments, comment_limit, filter_type=filter_type
                    )
                else:
                    logger.info(
                        f"No limiting needed: {len(filtered_comments)} comments <= limit of {comment_limit}"
                    )
            else:
                ...
                if len(filtered_comments) > max_comments:
                    logger.info(
                        f"Applying conservative limit: {len(filtered_comments)} -> {max_comments}"
                    )
                    filtered_comments = self._select_high_quality_comments(
                        filtered_comments, max_comments
                    )
    ...
    async def prepare_batch_requests(self):
        ...
        if not conversation_data:
            logger.error(
                "Failed to fetch conversation data. Cannot prepare batch requests."
            )
            return []
        ...
        for topic in topics:
            ...
            if is_global_section:
                ...
                logger.info(
                    f"Global section mapping - name: {topic_name}, filter_type: {filter_type}, "
                ...
                logger.info(
                    f"Topic mapping - cluster_id: {topic_cluster_id}, layer_id: {topic_layer_id}, "
                    f"topic_name: {topic_name}, topic_key: {topic_key}"
                )

            # Get comments as XML
            structured_comments = await self.get_comments_as_xml(
                conversation_data, self.filter_topics, filter_args
            )
            ...
            if topic_cluster_id == 0 or str(topic_cluster_id) == "0":
                ...
                logger.info(
                    f"DEBUG: Topic 0 structured_comments length: {len(structured_comments) if structured_comments else 0}"
                )
                logger.info(
                    f"DEBUG: Topic 0 has content: {bool(structured_comments and structured_comments.strip())}"
                )

            # Skip if no structured comments
            if not structured_comments.strip():
                logger.warning(
                    f"No content after filter for topic {topic_name} (cluster_id={topic_cluster_id})"
                )
                continue
            ...
            if is_global_section:
                ...
                if section_name.endswith("_groups"):
                    ...
                    logger.warning(
                        f"Could not determine base name from section_name '{section_name}', using fallback: '{base_name}'"
                    )

                template_filename = template_mapping.get(base_name, "topics.xml")
                template_path = (
                    self.prompt_base_path / f"subtaskPrompts/{template_filename}"
                )

                logger.info(
                    f"Using template {template_filename} for global section {section_name} (base_name: {base_name})"
                )
            else:
            ...
            try:
                ...
                template_dict["polisAnalysisPrompt"]["data"] = {
                    "content": {"structured_comments": structured_comments}
                }

                # Add topic name to prompt
                if "context" in template_dict["polisAnalysisPrompt"]:
                    if isinstance(
                        template_dict["polisAnalysisPrompt"]["context"], dict
                    ):
                        template_dict["polisAnalysisPrompt"]["context"][
                            "topic_name"
                        ] = topic_name
    ...
    async def process_request(self, request):
        ...
        try:
            ...
            section_name = metadata.get(
                "section_name", f"topic_{topic_name.lower().replace(' ', '_')}"
            )
            ...
            if self.report_id:
                ...
                logger.warning(
                    f"No report_id available, skipping storage for {section_name}"
                )
    ...
    async def submit_batch(self):
        ...
        try:
            ...
            if not batch_requests:
                logger.error(
                    "No batch requests to submit - prepare_batch_requests returned empty list"
                )
                return None
        ...
        # Log job information
        logger.info(
            f"Processing batch of {len(batch_requests)} requests for conversation {self.conversation_id}"
        )
        if self.job_id:
        ...
        if not anthropic_api_key:
            logger.error(
                "ERROR: ANTHROPIC_API_KEY environment variable is not set. Cannot submit batch."
            )
            if self.job_id:
                try:
                    ...
                    logger.info(
                        f"Updated job {self.job_id} status to FAILED due to missing API key"
                    )
                except Exception as e:
        ...
        try:
            ...
            try:
                for i, request in enumerate(batch_requests):
                    ...
                    if self.job_id and self.job_id in section_name:
                        ...
                        shortened_section = section_name.replace(
                            self.job_id, short_job_id
                        )
                    ...
                    logger.info(
                        f"Custom ID construction: conversation_id={self.conversation_id}, section_name='{section_name}', custom_id='{custom_id}', safe_custom_id='{safe_custom_id}'"
                    )
                    ...
                    if len(safe_custom_id) > 64:
                        ...
                        logger.warning(
                            f"Truncated custom_id to 64 chars: {safe_custom_id}"
                        )
                    ...
                    if not system_content:
                        logger.warning(
                            f"Empty system prompt for request {i}, using default"
                        )
                        system_content = (
                            "You are a helpful AI assistant analyzing survey data."
                        )
                ...
                logger.info(
                    f"Successfully formatted {len(formatted_batch_requests)} batch requests"
                )
                ...
                if formatted_batch_requests:
                    ...
                    if "params" in debug_request:
                        ...
                        if (
                            "system" in debug_request["params"]
                            and isinstance(debug_request["params"]["system"], str)
                            and len(debug_request["params"]["system"]) > 100
                        ):
                            debug_request["params"]["system"] = (
                                debug_request["params"]["system"][:100]
                    ...
                    logger.info(
                        f"Sample batch request structure: {json.dumps(debug_request, indent=2)}"
                    )
                    logger.info(
                        "Using format that matches working example from other project"
                    )
            ...
            logger.info(
                f"Submitting {len(formatted_batch_requests)} requests to Anthropic Batch API"
            )

            # Submit the batch to Anthropic with detailed error handling
            try:
                batch = anthropic.beta.messages.batches.create(
                    requests=formatted_batch_requests
                )
                ...
                logger.error(
                    f"Unexpected error submitting batch to Anthropic API: {str(e)}"
                )
                logger.error(traceback.format_exc())
                return None
            ...
            if self.job_id:
                logger.info(
                    f"Updating job {self.job_id} with batch information in DynamoDB..."
                )
                try:
                    job_table = self.dynamodb.Table("Delphi_JobQueue")
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to connect to Delphi_JobQueue table: {str(e)}"
                        )
                        ...
                        return (
                            batch.id
                        )  # Still return batch ID even if we can't update DynamoDB
                    ...
                    logger.info(
                        f"Attempting to store batch_id as string: {batch_id_str}"
                    )
                    ...
                    if "Item" in verify_job:
                        ...
                        if "batch_id" in job_item:
                            logger.info(
                                f"VERIFICATION SUCCESS: batch_id found in job record: {job_item['batch_id']}"
                            )
                        else:
                            logger.error(
                                "VERIFICATION FAILED: batch_id not found in job record!"
                            )
                    ...
                    logger.info(
                        f"Successfully updated job {self.job_id} with batch information"
                    )
                    logger.info(f"Batch ID: {batch.id} stored in job record")
                    logger.info(f"DynamoDB update response: {update_response}")
                    logger.info(
                        "Job is now in PROCESSING state - poller will run batch status checks"
                    )
                    ...
                    try:
                        ...
                        status_check_job_id = (
                            f"batch_check_{self.job_id}_{int(time.time())}"
                        )
                        ...
                        logger.info(
                            f"Scheduled batch status check job {status_check_job_id} to run in 60 seconds"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to schedule batch status check job: {str(e)}"
                        )
                        logger.error(traceback.format_exc())
                        # Continue despite failure
                        logger.info(
                            "Continuing despite failure to schedule status check job"
                        )

                except Exception as e:
                    logger.error(
                        f"Failed to update job with batch information: {str(e)}"
                    )
                    logger.error(traceback.format_exc())
            ...
            if self.job_id:
                try:
                    ...
                    logger.info(
                        f"Updated job {self.job_id} status to FAILED due to error"
                    )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate narrative reports for Polis conversations"
    )
    parser.add_argument(
        "--conversation_id",
        "--zid",
    ...
    parser.add_argument(
        "--no-cache", action="store_true", help="Ignore cached report data"
    )
    parser.add_argument(
        "--max-batch-size",
        type=int,
