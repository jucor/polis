...
def process_conversation(conversation_id: str) -> dict[str, Any]:
    ...
    if not comment_data:
        ...
        return {"success": False, "error": "No comments found", "conversation_id": conversation_id}
    ...
    conversation_meta = DataConverter.create_conversation_meta(conversation_id, embeddings, cluster_layers, metadata)
    ...
    for i in range(len(projection)):
        ...
        for j, (neighbor_idx, distance) in enumerate(zip(nearest_indices, nearest_distances, strict=False)):
            ...
            edge = DataConverter.create_graph_edge(
                conversation_id, comment_id_i, comment_id_j, distance, distance, True, shared_layers
            )
...
def process_new_comment(comment_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if not all([conversation_id, comment_id, text]):
        ...
        return {"success": False, "error": "Missing required fields", "comment_data": comment_data}
    ...
    cluster_layers = clustering_engine.create_clustering_layers(all_vectors, num_layers=4)
    ...
    cluster_model = CommentCluster(conversation_id=conversation_id, comment_id=int(comment_id), **new_clusters)
    ...
    try:
        ...
        if event_type == "process_conversation":
            ...
            if not conversation_id:
                return {"statusCode": 400, "body": json.dumps({"error": "Missing conversation_id", "event": event})}
            ...
            if not comment_data:
                return {"statusCode": 400, "body": json.dumps({"error": "Missing comment_data", "event": event})}
            ...
            return {
                "statusCode": 400,
                "body": json.dumps({"error": f"Unknown event_type: {event_type}", "event": event}),
            }
        ...
        return {
            "statusCode": 500,
            "body": json.dumps({"error": str(e), "trace": traceback.format_exc(), "event": event}),
        }
