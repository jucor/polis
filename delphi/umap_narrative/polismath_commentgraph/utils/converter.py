...
class DataConverter:
    ...
    @overload
    @staticmethod
    def convert_floats_to_decimal(obj: dict[str, Any]) -> dict[str, Any]: ...

    @overload
    @staticmethod
    def convert_floats_to_decimal(obj: list[Any]) -> list[Any]: ...

    @overload
    @staticmethod
    def convert_floats_to_decimal(obj: float) -> Decimal: ...

    @overload
    @staticmethod
    def convert_floats_to_decimal(obj: np.float32 | np.float64) -> Decimal: ...

    @overload
    @staticmethod
    def convert_floats_to_decimal(obj: np.int32 | np.int64) -> int: ...
    ...
    def convert_floats_to_decimal(
        ...
        This method preserves the structure of the input while converting numeric types:
        - float -> Decimal
        - np.float32/np.float64 -> Decimal
        - np.int32/np.int64 -> int
        - np.ndarray -> list
        - Recursively processes dicts and lists

        Args:
            obj: Input object to convert (dict, list, array, or scalar)

        Returns:
            Object with same structure but numeric types converted for DynamoDB compatibility
        """
    ...
    def create_conversation_meta(
        ...
        for i, layer in enumerate(cluster_layers):
            ...
            cluster_layer_info.append(ClusterLayer(layer_id=i, num_clusters=num_clusters, description=description))
    ...
    @staticmethod
    def create_comment_embedding(conversation_id: str, comment_id: int, vector: np.ndarray) -> CommentEmbedding:
        """
    ...
    def create_cluster_topic(
        ...
        parent_cluster: dict[str, int] | None = None,
        child_clusters: list[dict[str, int]] | None = None,
    ) -> ClusterTopic:
        """
        ...
        if child_clusters:
            child_refs = [
                ClusterReference(layer_id=child["layer_id"], cluster_id=child["cluster_id"]) for child in child_clusters
            ]
    ...
    def create_llm_topic_name(
        ...
        job_id: str | None = None,  # Added job_id
    ) -> LLMTopicName:
        """
    ...
    @staticmethod
    def batch_convert_embeddings(conversation_id: str, document_vectors: np.ndarray) -> list[CommentEmbedding]:
        """
    ...
    def batch_convert_umap_edges(
        ...
        for i in range(len(document_map)):
            ...
            node = DataConverter.create_graph_edge(
                ...
                shared_layers=[
                    layer_id for layer_id, layer in enumerate(cluster_layers) if i < len(layer) and layer[i] >= 0
                ],
    ...
    def batch_convert_clusters(
        conversation_id: str, cluster_layers: list[np.ndarray], document_map: np.ndarray
    ) -> list[CommentCluster]:
        """
                ...
                for comment_id in range(len(cluster_layers[0])):
                    if comment_id < len(cluster_layers[layer_id]) and comment_id < len(cluster_layers[parent_layer_id]):
                        ...
                        if current_cluster >= 0 and parent_cluster >= 0:
                            ...
                            if (
                                current_cluster
                                not in parent_child_map[f"layer{parent_layer_id}"]["children"][parent_cluster]
                            ):
                                parent_child_map[f"layer{parent_layer_id}"]["children"][parent_cluster][
                                    current_cluster
                                ] = 0
            ...
            for cluster_id in unique_clusters:
                ...
                if f"layer{layer_id}" in parent_child_map and "children" in parent_child_map[f"layer{layer_id}"]:
                    ...
                    if children:
                        # Get all children
                        child_clusters = [
                            {"layer_id": layer_id + 1, "cluster_id": int(child_id)} for child_id in children.keys()
                        ]
