...
1. **`delphi/docs/JOURNAL_OF_DISCREPANCIES_FIX.md`** — An ongoing (committed) tracking:
...
| FLI | 91,365 | **Need to generate** | Private |
| bg2018 | 226,233 | **Need to generate** | Private |
| pakistan | 400,076 | **Need to generate** | Private |
| engage | 442,584 | **Need to generate** | Private, has errors |
| bg2050 | 1,034,859 | **Need to generate** | Private, very large |
...
2. Run `generate_cold_start_clojure.py --all --include-local --pause-math` to generate cold-start blobs for FLI, bg2018, pakistan, engage, bg2050
...
4. Create `delphi/discrepancy_fix_journal.md` with initial baseline
...
3. Then **FLI** (91K), **bg2018** (226K), etc. — progressively larger
...
7. Document updated baseline in `discrepancy_fix_journal.md`
