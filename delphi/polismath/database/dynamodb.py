    ...
    def write_conversation(self, conv) -> bool:
        ...
        try:
            ...
            if projections_table and hasattr(conv, "proj"):
                ...
                for participant_id, coords in conv.proj.items():
                    ...
                    if len(participant_items) >= batch_size or processed_count == total_participants:
                        ...
                        if processed_count % log_interval == 0 or processed_count == total_participants:
                            ...
                            logger.info(
                                f"Written {processed_count}/{total_participants} participants ({progress_pct:.1f}%) - "
                ...
                logger.info(
                    f"Participant projection processing completed in {proj_time:.2f}s - "
    ...
    def write_projections_separately(self, conv) -> bool:
        ...
        try:
            ...
            if not conversations_table:
                logger.error("Delphi_PCAConversationConfig table not available")
                return False
            ...
            if not projections_table:
                logger.error("Delphi_PCAParticipantProjections table not available")
                return False
            ...
            logger.info(
                f"Processing {total_participants} projections in {len(chunks)} chunks with batch size {batch_size}"
            )
            ...
            for chunk_idx, chunk_keys in enumerate(chunks):
                ...
                logger.info(
                    f"Chunk {chunk_idx + 1}/{len(chunks)} completed in {chunk_time:.2f}s "
            ...
            logger.info(
                f"Projection processing completed in {total_time:.2f}s: "
            ...
            if total_success > 0:
                verification_response = projections_table.query(
                    KeyConditionExpression=boto3.dynamodb.conditions.Key("zid_tick").eq(zid_tick), Limit=5
                )
                ...
                logger.error("No projections were successfully written")
                return False
    ...
    def read_math_by_tick(self, zid: str, math_tick: int) -> dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "group_clusters": [],
                "proj": {},
                "repness": {"comment_repness": []},
                "vote_stats": {"comment_stats": {}},
                "comment_priorities": {},
            }
            ...
            if groups_table:
                response = groups_table.query(
                    KeyConditionExpression="zid_tick = :zid_tick", ExpressionAttributeValues={":zid_tick": zid_tick}
                )
                    ...
                    if "Items" in response:
                        for item in response["Items"]:
                            result["repness"]["comment_repness"].append(
                                {"gid": group_id, "tid": item.get("comment_id", ""), "repness": item.get("repness", 0)}
                            )
