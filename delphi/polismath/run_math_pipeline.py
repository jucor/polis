...
def main():
    ...
    try:
        ...
        for offset in range(0, min(total_votes, max_votes_to_process), batch_size):
            ...
            logger.info(
                f"[{time.time() - start_time:.2f}s] Database fetch completed in {db_fetch_time - batch_start_time:.2f}s"
            )
            ...
            logger.info(
                f"[{time.time() - start_time:.2f}s] Data transformation completed in {transform_time - db_fetch_time:.2f}s"
            )
        ...
        logger.info(
            f"[{time.time() - start_time:.2f}s] Clustering computation completed in {cluster_end - cluster_start:.2f}s"
        )
        ...
        logger.info(
            f"[{time.time() - start_time:.2f}s] Representativeness computation completed in {repness_end - repness_start:.2f}s"
        )
        ...
        info_end = time.time()
        logger.info(
            f"[{time.time() - start_time:.2f}s] Participant info computation completed in {info_end - info_start:.2f}s"
        )

        logger.info(
            f"[{time.time() - start_time:.2f}s] All computations complete! Total computation time: {time.time() - pca_start:.2f}s"
        )
