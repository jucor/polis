...
def main() -> None:
    ...
    math_command = [
        "python",
        "/app/polismath/run_math_pipeline.py",
        f"--zid={zid}",
    ]
    if max_votes_arg:
    ...
    math_process = subprocess.run(math_command, check=False)
    ...
    umap_command = [
        "python",
        "/app/umap_narrative/run_pipeline.py",
        f"--zid={zid}",
    ...
    pipeline_process = subprocess.run(umap_command, check=False)
    ...
    extremity_command = [
        "python",
        "/app/umap_narrative/501_calculate_comment_extremity.py",
        f"--zid={zid}",
        f"--include_moderation={args.include_moderation}",
    ]
    if verbose_arg:
    ...
    extremity_process = subprocess.run(extremity_command, check=False)
    ...
    priority_command = [
        "python",
        "/app/umap_narrative/502_calculate_priorities.py",
        f"--conversation_id={zid}",
    ]
    if verbose_arg:
        priority_command.append(verbose_arg)

    priority_process = subprocess.run(priority_command, check=False)
    ...
    if pipeline_exit_code == 0:
        ...
        for layer_id in available_layers:
            ...
            result = subprocess.run(datamap_command, check=False)
