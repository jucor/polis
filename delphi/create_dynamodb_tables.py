...
def create_evoc_tables(dynamodb: DynamoDBResource, delete_existing: bool = False) -> list[str]:
    ...
    tables: DynamoDBTableCollection = {
        ...
        "Delphi_CollectiveStatement": {
            "KeySchema": [{"AttributeName": "zid_topic_jobid", "KeyType": "HASH"}],
            "AttributeDefinitions": [
                ...
                for gsi in table_schema["GlobalSecondaryIndexes"]:
                    for key_element in gsi["KeySchema"]:
                        ...
                        if not is_defined:
                            ...
                            logger.warning(
                                f"Attribute {attr_name} for GSI in {table_name} was not in AttributeDefinitions. Ensure it's added if not inferred."
                            )

            table = dynamodb.create_table(TableName=table_name, **table_schema)
...
def create_tables(
    ...
    if (
        ...
        and (
            ...
            or "host.docker.internal" in endpoint_url
            or "polis-dynamodb-local" in endpoint_url
        )
    ):
