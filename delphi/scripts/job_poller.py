...
class JobProcessor:
    ...
    def find_pending_job(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            def execute_paginated_query(status: str) -> list[dict[str, Any]]:
                items = []
                last_key = None
                while True:
                    query_kwargs = {
            ...
            logger.info(
                f"Found {len(actionable_jobs)} actionable job(s). Highest priority is {actionable_jobs[0]['job_id']}"
            )
    ...
    def get_job_actual_size(self, conversation_id_str: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error querying PostgreSQL for comment count (conv_id: {conversation_id_str}): {e}. Defaulting to 'normal' size."
            )
            return "normal"
        finally:
            if pg_client:
    ...
    def complete_job(
        ...
        try:
            ...
            try:
                ...
                if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                    logger.warning(
                        f"Job {job_id} was modified by another process, completion state may not be accurate"
                    )
                else:
                    raise
        except Exception as e:
    ...
    def process_job(self, job: dict[str, Any]) -> None:
        ...
        self.update_job_logs(
            ...
            {
                "level": "INFO",
                "message": f"Worker {self.worker_id} starting job {job_id}",
            },
        )
        ...
        try:
            ...
            if job_type == "CREATE_NARRATIVE_BATCH":
                ...
                cmd = [
                    ...
                    f"--model={model}",
                    f"--include_moderation={include_moderation}",
                    f"--max-batch-size={str(max_batch_size)}",
                ]
                if job_config.get("no_cache"):
                    cmd.append("--no-cache")
                ...
                if report_id:
                    ...
                    self.update_job_logs(
                        ...
                        {
                            "level": "INFO",
                            "message": f"Passing report_id {report_id} to run_delphi.py",
                        },
                    )
            ...
            if job_type == "AWAITING_NARRATIVE_BATCH":
                ...
                if success:
                    ...
                    self.complete_job(
                        job,
                        False,
                        error=f"CREATE_NARRATIVE_BATCH script failed with exit code {return_code}",
                    )
    ...
    while running:
        ...
        try:
            ...
            if job_to_process:
                ...
                if not can_process:
                    logger.info(
                        f"Worker instance type '{instance_type}' cannot process job '{job_to_process['job_id']}' of size '{job_actual_size}'. Skipping for now."
                    )
            ...
            logger.error(
                f"Critical error in polling loop for worker {processor.worker_id}: {e}",
                exc_info=True,
            )
            if claimed_job:
                processor.complete_job(
                    claimed_job,
                    False,
                    error="Polling loop crashed during processing",
                )
